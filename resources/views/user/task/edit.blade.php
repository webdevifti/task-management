    ...
    <main id="main" class="main">
                ...
                <div>
                    ...
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('user.tasks.index') }}">Tasks</a></li>
                            <li class="breadcrumb-item active">Edit The Task</li>
                        </ol>
                    </nav>
                </div>
                <a href="{{ route('user.tasks.index') }}" class="btn btn-outline-primary btn-sm"><i class="bi bi-arrow-left"></i></a>
            </div>
        ...
        <section class="section dashboard">
            <div class="row">
                <div class="card p-2">
                    <div class="card-body">
                        <form id="taskUpdateForm" enctype="multipart/form-data" method="POST">
                            ...
                            <div class="mb-2">
                                ...
                                <textarea cols="30" rows="5" name="description" placeholder="Description" class="form-control" required>{{ $task->description }}</textarea>
                                @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="mb-2">
                                ...
                                <input type="radio" name="status" {{ $task->status == 'Pending' ? 'checked' : '' }}
                                    value="Pending" id="Pending">
                                <label for="Pending">Pending</label>
                                <input type="radio" name="status" {{ $task->status == 'In Progress' ? 'checked' : '' }}
                                    value="In Progress" id="In Progress">
                                <label for="In Progress">In Progress</label>
                                <input type="radio" name="status" {{ $task->status == 'Completed' ? 'checked' : '' }}
                            ...
                            <div class="mb-3">
                                @if ($task->image)
                                    <img src="{{ asset($task->image) }}" width="100" alt="">
                                    <br>
                                @endif
                                @error('image')
                                    <span class="text-danger">{{ $message }}</span>
