    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle">
            <div class="d-flex align-items-center justify-content-between">
                ...
                <a href="{{ route('user.tasks.create') }}" class="btn btn-outline-primary btn-sm">Add New</a>
            </div>
        ...
        <section class="section dashboard">
            <div class="row">
                <form action="{{ route('user.tasks.index') }}" method="GET" id="filterForm">
                    <div class="row mb-3">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-2 d-flex align-items-center">
                            ...
                            <a href="{{ route('user.tasks.index') }}" class="btn btn-danger w-100">Clear</a>
                        </div>
                    </div>
                </form>
                        ...
                        @if ($tasks->count() > 0)
                            ...
                            <tbody id="tasksTableBody">
                                @foreach ($tasks as $item)
                                    <tr id="row{{ $item->id }}">
                                        <td><a
                                                href="{{ route('user.tasks.show', $item->id) }}">#{{ $item->task_number }}</a>
                                        </td>
                                        <td>{{ $item->title }}</td>
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('user.tasks.edit', $item->id) }}"
                                                class="btn btn-outline-success btn-sm"><i class="bi bi-pencil-square"></i></a>
                                            <a href="{{ route('user.tasks.show', $item->id) }}"
...
@section('scripts')
    <script>
        ...
        $('#statusConfirmation').on('click', '#confirmUpdate', function() {
            ...
            $.ajax({
                url: `/user/tasks/status/update/${taskId}`,
                method: 'POST',
                data: {
                    status: newStatus,
                    _token: '{{ csrf_token() }}',
                ...
                success: function(res) {
                    ...
                    if (tasks.length > 0) {
                        tasks.forEach(item => {
                            taskRows += `
                            <tr id="row${item.id}">
                                <td><a href="/user/tasks/${item.id}">#${item.task_number}</a></td>
                                <td>${item.title}</td>
                                <td>${item.description}</td>
                                <td>
                                ...
                                <td>
                                    <button data-task-id='${item.id}' data-content='${JSON.stringify(item)}' data-bs-toggle="modal"
                                        data-bs-target="#statusConfirmation"
                                        class="btn btn-primary btn-sm statusBtn"><i class="bi bi-pencil-fill"></i></button>
                                    <a href="/user/tasks/${item.id}/edit"
                                        class="btn btn-outline-success btn-sm"><i class="bi bi-pencil-square"></i></a>
                                    <a href="/user/tasks/${item.id}"
                                        class="btn btn-outline-primary btn-sm"><i class="bi bi-eye-fill"></i></a>
